...
- Keep one typography system across the collection. A themed color palette, image treatment, or decorative surface may vary by destination, but heading/body font families must match the collection reference unless the user explicitly approves a different direction. Do not treat automatic CJK fallback differences as an acceptable substitute for declaring the shared stack.
...
- Keep the initial map fitted to the local itinerary. Provide a visible `航班` layer toggle whose `aria-pressed` value matches both the flight-route and airport-layer visibility. Turning it on should fit the air journey; turning it off should hide both layers and return to the ground route. Flight legend entries must restore hidden flight layers before fitting/opening the selected route. Reserve space for map overlays so neither endpoint is hidden behind the toolbar or legend. On narrow screens, collapse the large route legend while a flight is focused and restore it when returning to the ground route.
